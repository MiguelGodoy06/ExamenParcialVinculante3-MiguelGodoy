...
void Launcher::asignarTareasConInput() {
    for (auto& actor : actores) {
        while (true) {
            ...
            // Búsqueda de la tarea por ID
            auto it = find_if(tareas.begin(), tareas.end(), [&](const Tarea& tarea) {
                return tarea.getId() == tareaId;
            });

            if (it != tareas.end()) {
                actor.addTarea(*it); // Asignar la tarea encontrada
            } else {
...
void Launcher::mostrarTareas() {
    imprimirTareas(); // Reutilizamos la función imprimirTareas
}

// Nueva función para iniciar el sistema
void Launcher::iniciar() {
    ejecutar(); // Reutilizamos la función ejecutar
}
